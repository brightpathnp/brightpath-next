...
const row = (label: string, value: string): string => `
  <tr>
    <td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; width: 190px; vertical-align: top; font-size: 13px; color: #64748b; font-weight: 600;">
      ${label}
    </td>
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const englishTest = body.englishTest?.trim() ?? 'no';
    const testType = body.testType?.trim() ?? '';
    const testScore = body.testScore?.trim() ?? '';
    ...
    const englishTestSummary =
      englishTest === 'yes'
        ? `${formatValue(testType || 'Test given')} (${formatValue(testScore || 'Score not provided')})`
        : 'Not given';
        ...
        <div style="max-width: 720px; margin: 0 auto; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden;">
          <div style="background: linear-gradient(135deg, #0f172a 0%, #1d4ed8 55%, #7c3aed 100%); padding: 28px 32px;">
            ...
            <p style="margin: 10px 0 0; font-size: 14px; line-height: 1.6; color: #dbeafe;">
              A new consultation submission was received from the website.
            </p>
          </div>
          ...
          <div style="padding: 32px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              ${row('Form Type', formatValue(formType))}
              ...
              ${row('City', formatValue(city))}
              ${row('Highest Qualification', formatValue(qualification))}
              ${row('Interested Destination', formatValue(destination))}
              ${row('Preferred Date', formatValue(preferredDate))}
              ${row('Preferred Time', formatValue(preferredTime))}
              ${row('English Test Status', englishTestSummary)}
            </table>
      ...
      <div style="margin: 0; padding: 24px; background: #f3f4f6; font-family: Arial, sans-serif; color: #111827;">
        <div style="max-width: 640px; margin: 0 auto; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden;">
          <div style="background: linear-gradient(135deg, #0f172a 0%, #1d4ed8 55%, #7c3aed 100%); padding: 28px 32px;">
            ...
            <h1 style="margin: 0; font-size: 24px; line-height: 1.3; color: #ffffff;">
              ${isCallBack ? 'Your call back request is confirmed' : 'Your consultation request is received'}
            </h1>
            <p style="margin: 10px 0 0; font-size: 14px; line-height: 1.6; color: #dbeafe;">
          ...
          <div style="padding: 32px;">
            ...
            <p style="margin: 0 0 16px; font-size: 15px; line-height: 1.8; color: #334155;">
              We have received your ${isCallBack ? 'call back request' : 'consultation request'} successfully.
            </p>
            ...
            <div style="margin: 20px 0; padding: 18px 20px; background: #f8fafc; border: 1px solid #e5e7eb; border-radius: 12px;">
              ...
              <p style="margin: 0 0 6px; font-size: 14px; color: #0f172a;"><strong>City:</strong> ${formatValue(city)}</p>
              <p style="margin: 0 0 6px; font-size: 14px; color: #0f172a;"><strong>Destination:</strong> ${formatValue(destination)}</p>
              <p style="margin: 0 0 6px; font-size: 14px; color: #0f172a;"><strong>Qualification:</strong> ${formatValue(qualification)}</p>
              <p style="margin: 0; font-size: 14px; color: #0f172a;"><strong>Preferred Schedule:</strong> ${formatValue(preferredDate)}${preferredTime ? ` at ${escapeHtml(preferredTime)}` : ''}</p>
            </div>
    ...
    if (adminResult.error) {
      console.error('Resend consultation admin email error:', adminResult.error);
      return NextResponse.json(
        { error: 'Failed to send admin consultation email.', details: adminResult.error },
        { status: 500 }
      );
    }
    ...
    const userResult = await resend.emails.send({
      ...
      subject: isCallBack
        ...
        : 'Your consultation request is received — BrightPath Nepal',
      html: userHtml,
    });

    if (userResult.error) {
      console.error('Resend consultation confirmation email error:', userResult.error);
      return NextResponse.json(
        {
